var dealSaleRequire ={
		...
		pageInit:function(){
			$.post(ext.path+'require/getRequireByRequireid.do',{requireid:dealSaleRequire.vars.requireid},function(data){
				console.log(data);
				if(data){
					$("#address_span").html(data.city+data.district+data.street);
					$("#usedto_span").html(data.usedto);
					$("#decorate_span").html(data.decorate);
					$("#price_span").html(data.price+"万元");
