		...
		$.post(ext.path+"house/getRandAgent.do",{storeid:ext.storeId},function(data){
			if(data.length>0){
				...
				$('.sureModifyBtn').click(function(){
					...
					if(agentopenid != null){
						$.post(ext.path+"require/sendRequireMsg.do",{"requireid":requireEntrust.requireid,"agentopenid":agentopenid,"storeid":ext.storeId},function(data){
							location.href=ext.path+"page/requireSuccess.do";
						});
					} else {
						alert("请选择委托经纪人。");
					}
